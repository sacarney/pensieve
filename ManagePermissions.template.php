...
function template_permission_index()
{
      ...
      <table class="table is-narrow is-bordered is-fullwidth">
        <thead>
          <tr class="catbg">
            ...
            <th>', $txt['membergroups_members_top'], '</th>';

      if (empty($modSettings['permission_enable_deny']))
        echo '
            <th>', $txt['membergroups_permissions'], '</th>';
      else
        echo '
            <th>', $txt['permissions_allowed'], '</th>
            <th>', $txt['permissions_denied'], '</th>';

      echo '
            <th>', $context['can_modify'] ? $txt['permissions_modify'] : $txt['permissions_view'], '</th>
  ...
  foreach ($context['groups'] as $group)
  {
    ...
    if (empty($modSettings['permission_enable_deny']))
      ...
      echo '
            ...
            <td ', $group['id'] == 1 || $group['id'] == -1 ? ' style="font-style: italic;"' : (!empty($group['num_permissions']['denied']) ? ' style="color: red;"' : ''), '>', $group['num_permissions']['denied'], '</td>';
    ...
    echo '
            ...
            <td style="vertical-align: middle; text-align: center;">', $group['allow_modify'] && $context['can_modify'] ? '<input type="checkbox" name="group[]" value="' . $group['id'] . '" class="input_check" />' : '', '</td>
          </tr>';
  }
  ...
  if ($context['can_modify'])
  {
        ...
        <fieldset>
          ...
          <div class="field is-horizontal">
            <div class="field-label">
              <label class="label">', $txt['permissions_apply_pre_defined'], ' <a href="', $scripturl, '?action=helpadmin;help=permissions_quickgroups" onclick="return reqWin(this.href);">(?)</a></label>
            </div>
          ...
          <div class="field is-horizontal">
            <div class="field-label">
              <label class="label">', $txt['permissions_like_group'], '</label>
            </div>
            <div class="field-body">
              <div class="field">
                <div class="select">
    ...
    echo '
      ...
      <div class="mt-4">
        <input type="submit" value="', $txt['permissions_set_permissions'], '" onclick="return checkSubmit();" class="button is-small" />
      </div>';
...
function template_by_board()
{
  ...
  if (!$context['edit_all'])
    echo '
    <div class="mb-4">
      <a class="button button is-small" href="', $scripturl, '?action=admin;area=permissions;sa=board;edit;', $context['session_var'], '=', $context['session_id'], '">', $txt['permissions_board_all'], '</a>
    </div>';
  ...
  if ($context['edit_all'])
    echo '
      <input type="submit" name="save_changes" value="', $txt['save'], '" class="button is-small mt-4" />';
  else
    echo '
      <a class="button button is-small" href="', $scripturl, '?action=admin;area=permissions;sa=board;edit;', $context['session_var'], '=', $context['session_id'], '">', $txt['permissions_board_all'], '</a>';
...
function template_edit_profiles()
{
  ...
  <div id="admincenter">
    <form action="', $scripturl, '?action=admin;area=permissions;sa=profiles" method="post" accept-charset="', $context['character_set'], '">
      <div class="title_bar">
        <h3 class="title is-size-4 mb-3">', $txt['permissions_profile_edit'], '</h3>
      </div>
  ...
  foreach ($context['profiles'] as $profile)
  {
    ...
    if (!empty($context['show_rename_boxes']) && $profile['can_edit'])
      echo '
              <input type="text" name="rename_profile[', $profile['id'], ']" value="', $profile['name'], '" class="input is-auto" />';
    else
      echo '
  ...
  if ($context['can_edit_something'])
    echo '
        <input type="submit" name="rename" value="', empty($context['show_rename_boxes']) ? $txt['permissions_profile_rename'] : $txt['permissions_commit'], '" class="button is-small" />';

  echo '
        <input type="submit" name="delete" value="', $txt['quickmod_delete_selected'], '" class="button is-small" />
      </div>
    </form>
    <br />
    <form action="', $scripturl, '?action=admin;area=permissions;sa=profiles" method="post" accept-charset="', $context['character_set'], '">
      ...
      <div class="field is-horizontal">
        <div class="field-label">
          <label class="label">', $txt['permissions_profile_name'], '</label>
        </div>
        <div class="field-body">
          <div class="field">
            <input type="text" name="profile_name" value="" class="input is-auto" />
          </div>
        </div>
      </div>
      ...
      <div class="field is-horizontal">
        <div class="field-label">
          <label class="label">', $txt['permissions_profile_copy_from'], '</label>
        </div>
        <div class="field-body">
          <div class="field">
            <div class="select">
      ...
      <div class="mt-4">
        <input type="hidden" name="', $context['session_var'], '" value="', $context['session_id'], '" />
        <input type="submit" name="create" value="', $txt['permissions_profile_new_create'], '" class="button is-small" />
      </div>
    </form>
  </div>
  <br class="clear" />';
...
function template_modify_group()
{
  ...
  if ($context['profile']['can_modify'])
    echo '
      <div class="righttext padding">
        <input type="submit" value="', $txt['permissions_commit'], '" class="button is-small" />
      </div>';
...
function template_modify_group_simple($type)
{
  ...
  echo '
      <table width="100%" class="table is-narrow is-fullwidth">
        <thead>
          <tr class="catbg">
  ...
  foreach ($permission_data as $id_group => $permissionGroup)
  {
    ...
    if (!$permissionGroup['hidden'])
    {
      ...
      {
        echo '
          <tr class="background-overlay-dark">
        ...
        echo '
          <tr id="perm_div_', $id_group, '_', $permission['id'], '" class="', $alternate ? 'windowbg' : 'windowbg2', '">
            <td valign="top" width="10" style="padding-right: 1ex;">
              ', $permission['help_index'] ? '<a href="' . $scripturl . '?action=helpadmin;help=' . $permission['help_index'] . '" onclick="return reqWin(this.href);" class="help"><span class="fa fa-info-circle"></span><span class="sr-only">' . $txt['help'] . '</span></a>' : '', '
            </td>
      ...
      foreach ($permissionGroup['permissions'] as $permission)
      {
        ...
        else
        {
          echo '
              <tr class="', $alternate ? 'windowbg' : 'windowbg2', '">
                <td width="10">
                  ', $permission['show_help'] ? '<a href="' . $scripturl . '?action=helpadmin;help=permissionhelp_' . $permission['id'] . '" onclick="return reqWin(this.href);" class="help"><span class="fa fa-info-circle"></span><span class="sr-only">' . $txt['help'] . '</span></a>' : '', '
                </td>';
...
function template_postmod_permissions()
{
  ...
  <div id="admincenter">
    <form action="', $scripturl, '?action=admin;area=permissions;sa=postmod;', $context['session_var'], '=', $context['session_id'], '" method="post" name="postmodForm" id="postmodForm" accept-charset="', $context['character_set'], '">
      <div class="title_bar">
        <h3 class="title is-size-4 mb-3">', $txt['permissions_post_moderation'], '</h3>
      </div>';
  ...
  if (!empty($modSettings['permission_enable_deny']))
    echo '
        <div class="message is-danger">
          <div class="message-body">', $txt['permissions_post_moderation_deny_note'], '</div>
        </div>';
  ...
  <div class="field is-horizontal">
    <div class="field-label">
      <label class="label">', $txt['permissions_post_moderation_select'], '</label>
    </div>
    <div class="field-body">
